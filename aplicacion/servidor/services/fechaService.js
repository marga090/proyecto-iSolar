...
export const obtenerPorId = async (idCliente) => {
    ...
    const consultaActualizaciones = `
        SELECT 
            fecha_alta AS fecha, 
            'alta_cliente' AS ultimas_actualizaciones 
        FROM cliente 
        WHERE id_cliente = ?
        ...
        SELECT 
            fecha_legalizacion AS fecha, 
            'legalizacion_venta' AS ultimas_actualizaciones 
        FROM venta
        JOIN cliente ON venta.id_cliente = cliente.id_cliente 
        WHERE cliente.id_cliente = ? 
          AND fecha_legalizacion IS NOT NULL
        
        UNION ALL
        
        SELECT 
            fecha_instalacion AS fecha, 
            'instalacion' AS ultimas_actualizaciones 
        FROM instalacion
        ...
        SELECT 
            fecha_terminada AS fecha, 
            'instalacion_terminada' AS ultimas_actualizaciones 
        FROM instalacion
        JOIN venta ON instalacion.id_venta = venta.id_venta
        JOIN cliente ON venta.id_cliente = cliente.id_cliente 
        WHERE cliente.id_cliente = ? 
        ...
        SELECT 
            ...
            'registro_visita' AS ultimas_actualizaciones 
        FROM visita
        JOIN vivienda ON visita.id_vivienda = vivienda.id_vivienda
        JOIN domicilio ON vivienda.id_domicilio = domicilio.id_domicilio
        JOIN cliente ON domicilio.id_cliente = cliente.id_cliente 
